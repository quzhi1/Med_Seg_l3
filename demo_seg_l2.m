function demo_seg_l2(opt, numhid, ws, numpen, ws2, numhyper, ws3, maxiter, optvis, optmask, alpha)
clc;clear all;
addpath('./cnn');
addpath('./utils');
...
elseif strcmp(params.optimize, 'lbfgs'),
    params.fname = sprintf('%s_%s_rs_%d_cs_%d_ws_%d_nh_%d_ws2_%d_np_%d_%s_itr_%d', ...
        params.dataset, params.optimize, params.rs, params.cs, params.ws, params.numhid, params.ws2, params.numpen, params.nonlinearity, params.maxiter);
    
    if exist(sprintf('results/%s.mat', params.fname), 'file'),
        load(sprintf('results/%s.mat', params.fname), 'weights', 'params');
    else
        weights = cnn_l2_train_lbfgs(xtrain, ytrain, params, xval, yval);
    end
end


[~, ~, ap_val] = cnn_l2_evaluate(xval, yval, weights, params);
[~, ~, ap_test] = cnn_l2_evaluate(xtest, ytest, weights, params);
...
if optmask,
    if exist(sprintf('results/%s_al%g.mat', params.fname, alpha), 'file'),
        ...
        for i = 1:size(xtrain, 4),
            ...
            % inference
            pen = cnn_l2_infer(xc, weights, params);
